...
export default function Calls(){
  ...
  useEffect(()=>{ refresh() },[])
  const submit = async ()=>{ await api('/api/calls',{method:'POST', body: JSON.stringify({...form, booking_id: form.booking_id?Number(form.booking_id):null})}); setForm({booking_id:'', guest_name:'', phone:'', issue:'', notes:'', priority:'P2'}); refresh() }
  ...
  return (
    <div className="grid" style={{gap:16}}>
      <div className="card" style={{gridColumn:'span 5'}}>
        ...
        <div className="help">Track phone calls or text-based guest issues and resolutions.</div>
        <div className="row">
        ...
        <div className="row"><label>Guest name<input className="input" style={{flex:1}} value={form.guest_name} onChange={e=>setForm({...form, guest_name:e.target.value})} /></label></div>
        <div className="row"><label>Phone<input className="input" style={{flex:1}} value={form.phone} onChange={e=>setForm({...form, phone:e.target.value})} /></label></div>
        <div className="row"><label>Issue<input className="input" style={{flex:1}} value={form.issue} onChange={e=>setForm({...form, issue:e.target.value})} /></label></div>
        <div className="row"><label>Notes<textarea className="textarea" style={{width:'100%', minHeight:100}} value={form.notes} onChange={e=>setForm({...form, notes:e.target.value})} /></label></div>
