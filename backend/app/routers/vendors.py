...
def list_vendors():
  with SessionLocal() as db: return db.query(models.Vendor).order_by(models.Vendor.name.asc()).all()

@router.post("/", response_model=VendorOut)
def create_vendor(payload: VendorIn):
  with SessionLocal() as db:
    v = models.Vendor(**payload.model_dump()); db.add(v); db.commit(); db.refresh(v); return v
